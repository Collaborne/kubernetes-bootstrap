...
const argv = require('yargs')
	...
	.default('template-directory', 'templates').normalize('template-directory').describe('template-directory', 'Base directory for templates')
	.default('deploy-settings', 'deploy.yaml').describe('deploy-settings', 'Settings to use for processing the templates')
	.default('deploy-settings-overrides', 'deploy.yaml.override').describe('deploy-settings-overrides', 'Additional settings to load')
	.boolean('disable-overrides').describe('disable-overrides', 'Do not apply deploy.yaml.override')
	.default('output-directory', 'target').normalize('output-directory').describe('output-directory', 'Directory in which procesed templates are written')
	.boolean('output-only').describe('output-only', 'Only produce the output files, but do not apply them to the cluster')
	.default('kubeconfig', path.resolve(userDir, '.kube/config')).describe('kubeconfig', 'Kubectl configuration file to use for connecting to the cluster')
	.default('context', undefined).describe('context', 'Context in the kubectl configuration to use')
	.default('exclude', ['aws']).alias('x', 'exclude').describe('exclude', 'Template module to exclude')
	.default('define', []).alias('D', 'define').describe('define', 'Define/Override a setting on the command-line')
